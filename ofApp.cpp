...
void ofApp::draw() {
	...
	for (int x = -radius * 5; x <= radius * 5; x += radius * 2.5) {

		for (int y = -radius * 4; y <= radius * 4; y += radius * 4) {
			...
			int deg_span = 30;
			int p = ofRandom(720);
			int speed = ofRandom(6, 10);
			for (int i = 0; i < 13; i++) {

				int param = (p + ofGetFrameNum() * speed + i * deg_span) % 720;
